...
type CertificationRequest = Request<{}, {}, { certification_obj: TCertification }>;
 ...
 */
router.get("/", async (req: CertificationRequest, res: CertificationsResponse) => {
    ...
    req.log.debug({
        msg: "Getting all getCertifications",
        requesting_uuid: requesting_uuid,
    });

    // If the user is authorized, get all certification information
    if (await verifyRequest(requesting_uuid, API_SCOPE.GET_ALL_CERTIFICATIONS)) {
...
router.get(
    ...
    async (req: Request<{ UUID: string }>, res: CertificationResponse) => {
        ...
        if (
            ...
            if (!certification) {
                req.log.warn(`Certification not found by uuid ${certification_uuid}`);
 ...
 */
router.post("/", async (req: CertificationRequest, res: CertificationResponse) => {
    const headers = req.headers as VerifyRequestHeader;
    ...
    if (await verifyRequest(requesting_uuid, API_SCOPE.CREATE_CERTIFICATION)) {
        ...
        req.log.debug(`Created certification with uuid ${certification_uuid}`);
        res.status(StatusCodes.OK).json(certification);
    } else {
        req.log.warn({
 ...
 */
router.put("/", async (req: CertificationRequest, res: CertificationResponse) => {
    const headers = req.headers as VerifyRequestHeader;
    const requesting_uuid: string = headers.requesting_uuid;
    const certification_obj = req.body.certification_obj;
    const certification_uuid = certification_obj.uuid;
    ...
    if (await verifyRequest(requesting_uuid, API_SCOPE.UPDATE_CERTIFICATION)) {
...
router.delete(
    ...
    async (req: Request<{ UUID: string }>, res: CertificationResponse) => {
        ...
        if (await verifyRequest(requesting_uuid, API_SCOPE.DELETE_CERTIFICATION)) {
