...
// --- Developmental login methods ---
if (process.env.NODE_ENV === "development") {
    // Default login uses the autogenerated admin account.
...
// --- Administrator session login as other user ---
...
router.post("/login/email/", async (req: EmailLoginRequest, res, next) => {
    ...
    // Hash provided passkey
    const passkey_hash = createHash("sha256").update(passkey).digest("hex");

    // If the user has no passkey or the hashes are different, the request is invalid.
