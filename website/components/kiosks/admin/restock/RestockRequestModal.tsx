...
const updateRestocks = async ({
    ...
    if (isNew) {
        ...
        // Logic here for updating mailing list
        console.log("Updating", restock);
        console.log(restock.mailing_list)
        return (
            await axios.patch<TRestockRequest>(`/api/v3/restock/mailing_list/${restock.uuid}`, {
...
export default function RestockRequestModal({
    ...
    const prevRestock = restocks.findLast((r) => r.item_uuid == restockSelected.uuid);

    // is a new restock if it isn't in the list, if it is on the list, its status isn't restock or pending

    const isNew = !prevRestock || (prevRestock.current_status === RESTOCK_REQUEST_STATUS.RESTOCKED) || (prevRestock.current_status === RESTOCK_REQUEST_STATUS.DENIED);

    const userCanRequest = prevRestock ? !(prevRestock.mailing_list.includes(requestingUser.uuid) || (prevRestock.requesting_user == requestingUser.uuid)) : true;
                            ...
                            <ModalBody>
                                <Form onSubmit={onSubmit}>
                                    {isNew ?
                                            ...
                                            <div className="w-full flex flex-col gap-2">
                                                <div className='flex flex-row gap-2'>
                                                    <Input
                                                        ...
                                                        isDisabled
                                                        defaultValue={restockSelected.name}
                                                        variant="faded"
                                                        color="primary"
                                                        size="md"
                                                    />
                                        ...
                                        :
                                        <div>
                                            <p className='pb-2'>There is already a restock request for this item. If you would like to be notified when it is restocked, please select "Add to Mailing List" below.</p>

                                            <div className='flex flex-col gap-2'>
                                                <div className='flex flex-row gap-2 items-center '>
                                                    <h3 className='font-bold'>Current Status: </h3>
                                                    <RestockType request_status={prevRestock.current_status} size="md" />
                                                </div>
                                                <div className='flex flex-row gap-2 items-center '>
                                                    <h3 className='font-bold'>Last Message: </h3>
                                                    <p>{prevRestock.status_logs.at(-1) ? prevRestock.status_logs.at(-1)?.message : ""}</p>
                                                </div>

                                                {userCanRequest
                                                    ? null
                                                    : <div className='flex justify-center'>
                                                        <p className='font-bold'>You are already on the mailing list.</p>
                                                    </div>
                                                }
                                            </div>
                                            <ModalFooter className="w-full justify-between">
