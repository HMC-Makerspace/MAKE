...
export default function WorkshopEditModal({
        ...
        if (users) {
            const stewards = users.filter((user) => {
                return user.active_roles.some((role) => {
                    return !(
                        role.role_uuid ===
                        "5ff61ecc-30db-45e8-88d5-2db406fe5e06"
                    );
                });
            });
            stewards.sort((a, b) => {
    ...
    const [requiredCertifications, setRequiredCertifications] = React.useState<
        CertificationUUID[]
